...
namespace TalentGoWebApp.Areas.Mgmt.Controllers
{
	...
	public class RecruitmentPlanController : Controller
	{
		...
		protected override void Initialize(RequestContext requestContext)
		{
			...
			this.recruitmentManager = new RecruitmentManager(requestContext.HttpContext);
		}
		// GET: Mgmt/Recruitment
		public async Task<ActionResult> Index(int? id)
		{
			return View(await this.recruitmentManager.GetAllRecruitmentPlans(id));
		}
		...
		{
			var current = this.database.RecruitmentPlan.SingleOrDefault(e => e.id == id && !e.WhenPublished.HasValue);
			if (current == null)
			{
				return RedirectToAction("Index");
			}

			this.database.RecruitmentPlan.Remove(current);
			this.database.SaveChanges();
			return RedirectToAction("Index");

			//try
			//{
			//    var link = from plan in this.database.ArchiveRequirements
			//               where plan.RecruitmentPlanID == id
			//               select plan;
			//    if (link != null)
			//    {
			//        this.database.ArchiveRequirements.RemoveRange(link);
			//    }
			//    var current = this.database.RecruitmentPlan.SingleOrDefault(e => e.id == id);
			//    if (current != null)
			//    {
			//        this.database.RecruitmentPlan.Remove(current);
			//    }
		...
		{
			RecruitmentPlan plan = await this.recruitmentManager.FindByID(id);
			if (plan == null)
				return HttpNotFound();
		...
		public async Task<ActionResult> Edit(int id, RecruitmentPlanPrimaryViewModel model)
		{
			if (ModelState.IsValid)
			{
				var plan = await this.recruitmentManager.FindByID(id);
		...
		public async Task<ActionResult> ArchiveRequirements(int id, List<ArchiveRequirementsViewModel> model)
		{
			...
			foreach (ArchiveRequirementsViewModel item in model)
			{
				...
				if (item.Enabled)
				{
					...
					if (orgselected == null)
						this.database.ArchiveRequirements.Add(new ArchiveRequirements()
						{
					...
					{
						if (orgselected.Requirements != item.RequirementType)
				...
				else
				{
					...
					if (orgselected != null)
						this.database.ArchiveRequirements.Remove(orgselected);
				}
		...
		public async Task<ActionResult> Publish(int id)
		{
			...
			{
				Plan = await this.recruitmentManager.FindByID(id)
			};
			return View(model);
		}
