...
namespace TalentGo.Recruitment
{
    ...
    public class EnrollmentManager
    {
		...
		public EnrollmentManager(IEnrollmentStore Store, RecruitmentPlanManager recruitmentPlanManager, TargetUserManager targetUserManager)
        {
            this.store = Store;
        ...
        /// <param name="plan"></param>
        /// <returns></returns>
        public async Task<Enrollment> NewEnrollment(TargetUser user, RecruitmentPlan plan)
        {
            //根据当前需求，不允许存在多个报名表。
        ...
        /// <returns></returns>
        public async Task CreateEnrollment(TargetUser user, RecruitmentPlan plan, Enrollment enrollment)
        {
            //根据要求，一个用户只能参与一个报名。
            if (this.Enrollments.Any(e => e.UserID == user.Id))
        ...
        public async Task UpdateEnrollment(TargetUser user, RecruitmentPlan plan, Enrollment enrollment)
        {
#warning 需要进行检查和应用策略。
        ...
        /// <param name="enrollment"></param>
        /// <returns></returns>
        public async Task DeleteEnrollment(TargetUser user, RecruitmentPlan plan, Enrollment enrollment)
        {
            //如果该报名表已经提交，则不能删除。
            var currentuser = await this.targetUserManager.FindByIdAsync(user.Id);
            if (currentuser == null) throw new ArgumentException("user参数无效。");
            var currentplan = await this.recruitManager.FindByIDAsync(plan.id);
            if (currentplan == null) throw new ArgumentException("plan参数无效");
            if (enrollment.RecruitPlanID != currentplan.id || enrollment.UserID != currentuser.Id)
                throw new ArgumentException("enrollment参数无效。");

            if (this.CommitedEnrollments.Any(e => e.RecruitPlanID == enrollment.RecruitPlanID && e.UserID == enrollment.UserID))
        ...
        /// </summary>
        /// <returns></returns>
        public async Task CommitEnrollment(TargetUser user, RecruitmentPlan plan, Enrollment enrollment)
        {
        ...
        /// <returns></returns>
        public IQueryable<Enrollment> GetCommitedEnrollmentData(int PlanID, string MajorCategory, string CollegeName, AuditFilterType AuditFilter, AnnounceFilterType AnounceFilter, string Keywords)
        {
            //带分页
            //
            //先获得符合初始条件的集合
            ...
            if (!string.IsNullOrEmpty(CollegeName))
                initSet = initSet.Where(e => e.School.Contains(CollegeName));
        ...
        /// <returns></returns>
		public IQueryable<Enrollment> GetCommitedEnrollmentData(int PlanID, string MajorCategory, string CollegeName, AuditFilterType AuditFilter, AnnounceFilterType AnnounceFilter, string Keywords, string OrderColumn, bool DownDirection, out int ItemCount)
        {
            var resultSet = this.GetCommitedEnrollmentData(PlanID, MajorCategory, CollegeName, AuditFilter, AnnounceFilter, Keywords);
        ...
        /// <param name="ItemCount"></param>
        /// <returns></returns>
        public IQueryable<Enrollment> GetCommitedEnrollmentData(int PlanID, string MajorCategory, string CollegeName, AuditFilterType AuditFilter, AnnounceFilterType AnnounceFilter, string Keywords, string OrderColumn, bool DownDirection, int PageIndex, int PageSize, out int ItemCount)
        {
            var result = this.GetCommitedEnrollmentData(PlanID, MajorCategory, CollegeName, AuditFilter, AnnounceFilter, Keywords, OrderColumn, DownDirection, out ItemCount);
            if (ItemCount == 0)
            {
                return result;
            }
